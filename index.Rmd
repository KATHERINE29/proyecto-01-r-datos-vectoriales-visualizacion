...
author: "Katherine Urbina Téllez (B47122) y Jorge Masís Aguilar (B033742)"
date: "2020-10-07"
output:
  html_document:
    theme: readable    
...
#Crear dataframe para asia
asia_paises = world[world$continent == "Asia", ]
asia_ev = asia_paises[asia_paises$lifeExp >= "80", ]

 # Gráfico de barra para paíse del continente asiático con esparanza de vida de al menos 80 años.

ggplot(asia_ev, aes(name_long, lifeExp)) + 
  ggtitle("Esperanza de vida en Asia mayor o igual a 80 años") +
  xlab("País") + 
  ylab("Esperanza de vida") +   
    geom_col(aes(y=lifeExp), color = c("blue"))
```
...
```{r}
#Crear data frame para los países de Oceanía

oceania_paises = world[world$continent == "Oceania", ]

#Mapa de coropletas para la Esperanza de vida en el continente de Oceanía

plot(oceania_paises["lifeExp"], main="Esperanza de vida en países de Oceanía")
```
...
```{r}
library(dplyr)
#crear data frame paises de Centroamerica
Ca_paises <- world %>%
filter((subregion == "Central America" ) & (lifeExp >=74)) 

#mapa de coropletas para paises de América Central

plot(Ca_paises["lifeExp"], main = "Esperanza de Vida Centroamérica") 
```
...
paises_suramerica <- world %>%
filter((subregion == "South America" ) & (area_km2 <= 2500000 | pop <= 40000000)) %>%
select(name_long, area_km2, pop)

#Mapa de coropletas para países de Suramérica con área mayor a 2.5 km2

plot(paises_suramerica["area_km2"], main="Países Suramérica por área mayor a 2.5 km2")

#Mapa de coropletas para países de Suramérica con área mayor a 2.5 km2

plot(paises_suramerica["pop"], main="Países Suramérica con una población de 40 millones o más")
```